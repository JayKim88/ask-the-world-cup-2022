...
| 데모 목표 | BYOK(브라우저 저장 API Key) + **번들 읽기전용 SQLite**로 누구나 접속해 바로 써볼 수 있는 배포형 데모 |
| 시각화 | 기본 차트(막대/선/파이) + **D3 시그니처 시각화**(인터랙티브 토너먼트 대진표 트리 — 호버 시 경기 상세, 전환 애니메이션) |
| 스택 | Next.js 16(App Router) + React 19 + TS + Tailwind/shadcn + Vercel AI SDK + Zod + **SQLite** |
| 개발 기간 | 목표 2일, 초과 허용 |
...
- [ ] **Semantic-layer 방식 병행 구현** — LLM이 raw SQL이 아니라 **제약된 쿼리 플랜**(정의된 metric/dimension enum)을 뱉고, 결정적 컴파일러가 SQL을 생성하는 방식 (레퍼런스 `nl_to_plan_llm.py` 패턴). 프로덕션 분석 제품의 표준 접근을 손으로 익히는 목적.
  - 재사용: **같은 SQLite DB + 같은 §3.1 질문 뱅크 + 같은 eval 하버스**를 그대로 씀 → 코드만 NL→plan→SQL 경로 추가
...
실시간 연동이 아닌 **스냅샷 시딩** — 시딩 스크립트(`scripts/seed.ts`)가 위 엔드포인트를 1회 호출해 **로컬 SQLite 파일(`db/worldcup.db`)에 적재**(무료 티어 = 10 req/min·100 req/day → 6.5s 스로틀 + 일일 배치, 재개 가능). 그 `.db`는 **git에 커밋**되어 배포에 딸려간다. 이후 앱은 런타임에 API-Football을 호출하지 않고, `.db`를 **읽기전용으로 열어** 조회만 한다. eval 재현성(같은 gold set이 항상 같은 답)도 고정 스냅샷이라 보장.
...
| `DROP TABLE matches` 등 인젝션 시도 | AST 검증기 + SQLite 읽기전용 open 양쪽에서 차단되는지 (§7 DoD) |
...
| **DB = 번들 SQLite** (Supabase 아님) | 데이터가 런타임에 안 바뀌는 고정 읽기전용 스냅샷 → SQLite가 이상적. 클라우드 계정·role·RLS·RPC 하드닝이 전부 불필요해짐 | `db/worldcup.db`를 커밋→배포에 딸려감(런타임 DB 셋업 0). 읽기전용 open 하나로 무결성·격리. 대신 방언은 Postgres→SQLite (윈도우함수·재귀 CTE 둘 다 지원). 트레이드오프: 포트폴리오 신호는 Postgres가 더 실무적이나, 이 프로젝트 규모엔 SQLite 간결함이 우세 |
